"""
Atlas Browser Target - Browser automation for testing ChatGPT Atlas.
Uses Playwright to control the Atlas browser and test AI-specific vulnerabilities.
"""
import asyncio
import time
import logging
...
async def test_atlas_browser():
    """Test function to verify Atlas browser can be controlled."""
    target = AtlasBrowserTarget(headless=False)
    ...
    try:
        ...
        # Try to interact with sidebar
        response = await target.interact_with_sidebar("Hello, can you see this webpage?")
        if response:
            print(f"✓ Got AI response: {response[:100]}")
        else:
            print("⚠ Could not get AI response (sidebar might not be visible)")
...
if __name__ == "__main__":
    asyncio.run(test_atlas_browser())
